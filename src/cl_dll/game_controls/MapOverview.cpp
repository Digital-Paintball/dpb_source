...
static ConVar *mp_fadetoblack = NULL;
...
void CMapOverview::UpdatePlayerTrails()
{
	...
	for (int i=0; i<MAX_PLAYERS; i++)
	{
		...
		// If we can't track this player, kill his footsteps so we don't
		// have partial trails all over the place
		if ( !CanPlayerBeTracked( i ) )
		{
...
void CMapOverview::Paint()
{
	...
	if ( !mp_fadetoblack || !mp_fadetoblack->GetBool() )
	{
		DrawMapPlayerTrails();
...
/// returns false if (1) a player is dormant and (2) mp_forcecamera is not OBS_ALLOW_ALL
bool CMapOverview::CanPlayerBeTracked(int index)
{
	C_BasePlayer *player = UTIL_PlayerByIndex( index + 1 );
	if ( !player )
		return false;

	if ( player->IsDormant() && mp_forcecamera.GetInt() != OBS_ALLOW_ALL )
		return false;
...
void CMapOverview::DrawMapPlayers()
{
	...
	for (int i=0; i<MAX_PLAYERS; i++)
	{
		...
		if ( m_bShowHealth && CanPlayerHealthBeSeen( i ) )
		{
		...
		if ( m_bShowNames && CanPlayerNameBeSeen( i ) )
		{
...
{
	if ( !mp_fadetoblack )
		mp_fadetoblack = cvar->FindVar( "mp_fadetoblack" );
